/*
                Project 5: Calculating the SIMPLEX Algorithm
...
static GtkWidget* make_operator_combo(void) {
    ...
    gtk_widget_set_tooltip_text(combo, "Choose ≤, ≥ or =.");
...
static gboolean build_simplex_payload(SimplexUI *ui,
    ...
    // 1) read operators and count artificials (Big-M) and slack/excess columns
    int *restr_ops = g_new0(int, n_restr);
    int n_art = 0;      // artificials for >= or =
    int n_se  = 0;      // number of slack/excess columns (only for <= or >=)

    for (int r = 0; r < n_restr; ++r) {
        GtkComboBox *cmb = GTK_COMBO_BOX(g_ptr_array_index(ui->con_op_combos, r));
        int op = gtk_combo_box_get_active(cmb); // 0: <=, 1: >=, 2: =
        restr_ops[r] = op;

        if (op == OP_GEQ || op == OP_EQ)
            n_art++;           // artificial for >= or =
        if (op == OP_LEQ || op == OP_GEQ)
            n_se++;            // S or E for <= or >=, but NOT for =
    }

    // 2) matrix dimensions
    const int rows    = 1 + n_restr;
    const int cols    = 1 /*Z*/ + n_vars + n_se /*S/E*/ + n_art /*artificial*/ + 1 /*RHS*/;

    // column offsets
    const int z_col   = 0;
    const int var0    = 1;                // original variables x1..xn
    const int se0     = var0 + n_vars;    // S/E columns, only for <= or >=
    const int art0    = se0 + n_se;       // a1..a(k) (artificials)
    const int rhs_col = cols - 1;

    // map each constraint row r to its S/E column index, or -1 if none (for =)
    int *se_col_for_row = g_new0(int, n_restr);
    for (int r = 0; r < n_restr; ++r)
        se_col_for_row[r] = -1;

    int se_idx = 0;
    for (int r = 0; r < n_restr; ++r) {
        if (restr_ops[r] == OP_LEQ || restr_ops[r] == OP_GEQ) {
            se_col_for_row[r] = se0 + se_idx; // assign a column
            se_idx++;
        }
    }
    ...
    *problemName_out = g_strdup(pname ? pname : "");
    ...
    // 2) variable names
    int total_names = n_vars + n_se + n_art;
    char **varnames_all = g_new0(char*, total_names);

    // 1) decision variable names from the UI
    for (int j = 0; j < n_vars; ++j) {
        char buf[32];
        make_ascii_label('x', j + 1, buf);   // "x_1", "x_2", ...
        varnames_all[j] = g_strdup(buf);
    }

    // 2) S/E column names: S_i for <=, E_i for >=, none for =
    int se_name_idx = 0;
    int excess = 0;
    for (int r = 0; r < n_restr; ++r) {
        if (se_col_for_row[r] != -1) {
            char buf[32];
            if (restr_ops[r] == OP_LEQ) {
                make_ascii_label('S', r+1, buf);   // S_1, S_2, ...
            } else {
                make_ascii_label('E', excess+1, buf);   // E_1, E_2, ...
            }
            // this S/E column lives at index n_vars + se_name_idx
            varnames_all[n_vars + se_name_idx] = g_strdup(buf);
            se_name_idx++;
        }
    }

    // 3) artificial variable names a_1..a_k
    int art_idx = 0;
    for (int r = 0; r < n_restr; ++r) {
        if (restr_ops[r] == OP_GEQ || restr_ops[r] == OP_EQ) {
            char buf[32];
            make_ascii_label('a', art_idx+1, buf);   // a_1, a_2, ...
            varnames_all[n_vars + n_se + art_idx] = g_strdup(buf);
            art_idx++;
        }
    }
    ...
    *maximize_out = is_max;
    ...
    for (int j = 0; j < n_vars; ++j) {
        ...
        if (!parse_real_str(t, &v)) {
            free_names_array(varnames_all, total_names);
            g_free(restr_ops);
            return FALSE;
        }
    ...
    // slack/surplus block in objective row = 0
    for (int s = 0; s < n_se; ++s)
        M[0][se0 + s] = 0.0;

    // artificial variables: ±M depending on MAX/MIN
    for (int k = 0; k < n_art; ++k) {
        M[0][art0 + k] = is_max ? -BIG_M : BIG_M;
    }
    ...
    // 5) CONSTRAINT rows
    int art_col_counter = 0;  // for mapping rows → artificial columns

    for (int r = 0; r < n_restr; ++r) {
        // Z column = 0
        M[1 + r][z_col] = 0.0;

        // variable coefficients (x1..xn)
        for (int j = 0; j < n_vars; ++j) {
            GtkEntry *e = GTK_ENTRY(g_ptr_array_index(ui->con_coef_entries, r*n_vars + j));
            const gchar *t = gtk_entry_get_text(e);
            double v;
        ...
        // 1) initialize all S/E columns in this row to 0
        for (int s = 0; s < n_se; ++s)
            M[1 + r][se0 + s] = 0.0;

        // 2) set slack (+1) or excess (−1) only if this row has an S/E column
        int se_col = se_col_for_row[r];
        if (se_col != -1) {
            if (restr_ops[r] == OP_LEQ) {
                M[1 + r][se_col] = 1.0;    // slack
            } else if (restr_ops[r] == OP_GEQ) {
                M[1 + r][se_col] = -1.0;   // excess
            }
        }

        // 3) artificial variable column if this row needs one
        if (restr_ops[r] == OP_GEQ || restr_ops[r] == OP_EQ) {
            int col = art0 + art_col_counter;
            M[1 + r][col] = 1.0;   // a_k appears only in this constraint
            art_col_counter++;
        }
...
static void print_simplex_payload(double **A, const char *pname, char **vnames,
    ...
    printf("Var Names    : ");
    int total_names = cols - 2;   // instead of nvars + (rows - 1)
...
int main(int argc, char *argv[]) {
    ...
    GtkBuilder *builder = gtk_builder_new_from_file("programa-5.glade");
    GtkWidget  *win     = GTK_WIDGET(gtk_builder_get_object(builder, "program-5"));
